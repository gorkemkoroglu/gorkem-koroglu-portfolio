...
export const cases: Case[] = [
...

  {
    ...
    tr: {
      ...
      summary:
        "Manuel kontroller, tekrar eden veri girişi ve sınırlı statü görünürlüğü içeren mevcut operasyonel akışı analiz ederek; daha sade, izlenebilir ve sistem destekli hedef sürecin tasarlanmasına odaklanan süreç analizi vakası.",
      ...
      businessProblem:
        "Birden fazla ekip arasında ilerleyen (talep, onay, satın alma, finans) operasyonel bir süreç büyük ölçüde manuel adımlar, e-posta üzerinden ilerleyen handoff'lar ve tekrar eden veri girişiyle yürütülmektedir. İlerleme bilgisi tutarlı değildir, bazı kontroller kişilere bağımlıdır ve operasyon ekipleri bir gecikmenin nereden kaynaklandığını hızlıca ayırt edememektedir.",
      businessProblemBullets: [
        "Çok sayıda manuel handoff sorumluluğu belirsiz hale getirir",
        "Aynı bilgi farklı yerlerde tekrar girilir",
        "Onaylar sistem yerine e-posta üzerinden ilerler",
        "Ekipler arası statü görünürlüğü sınırlıdır",
        "Darboğaz ve gecikmeler kolay ölçülemez",
      ],
      analysisGoal:
        "Mevcut süreci uçtan uca görünür hale getirmek; rolleri, karar noktalarını ve bekleme alanlarını ayrıştırmak; tekrar eden veya gereksiz adımları tespit etmek; daha net bir hedef süreç önerisi hazırlamak; ve bu yapının test ve UAT açısından doğrulanabilir olmasını sağlamak.",
      analysisGoalItems: [
        ...
        "Test ve UAT'ta doğrulanabilir bir yapı kurmak",
      ],
      scope: [
        "Mevcut süreç adımlarının ve handoff'ların analizi",
        "Rol bazlı görev ve sorumluluk incelemesi",
        "Manuel ve tekrar eden işlerin tespiti",
        "Ekipler arası pain point analizi",
        "Hedef (TO-BE) süreç tasarımı önerisi",
        "Potansiyel sistem alanları, statü göstergeleri ve görünürlük ihtiyaçları",
        "Test ve UAT etkisinin değerlendirilmesi",
      ],
      outOfScope: [
        "Belirli bir araç/tedarikçi seçimi veya uygulaması",
        "Detaylı maliyet-fayda modellemesi",
        "Organizasyonel yeniden yapılanma kararları",
        "Uzun vadeli raporlama altyapısı tasarımı",
      ],
      currentState:
        "Mevcut süreç manuel bir talep ile başlar, e-posta üzerinden yönetici onayına gider, satın alma ekibinin sistem dışında yaptığı tedarikçi kontrolüyle devam eder ve sonunda finans tarafından geriye dönük olarak kayıt altına alınır. Aynı bilgi farklı noktalarda toplanır, statü gayri resmi şekilde paylaşılır ve sürecin tamamının izlenebildiği tek bir nokta yoktur.",
      currentStateBullets: [
        "Requester satın alma talebini manuel oluşturur",
        "Manager onayı e-posta veya sistem dışı kanalla verir",
        "Procurement tedarikçi uygunluğunu manuel kontrol eder",
        "Satın alma kaydı (PO) ayrı bir kayıt olarak oluşturulur",
        "Finance sürece yalnızca son kayıt aşamasında dahil olur",
        "Akış boyunca statü takibi sınırlı ve gayri resmidir",
      ],
      keyIssues: [
        { title: "Çok sayıda manuel handoff", desc: "Roller arası her geçiş manuel koordinasyon gerektirir; gecikme ve bilgi kaybı riski artar." },
        { title: "Onay iletişiminde gecikme", desc: "Onaylar yapılandırılmış bir iş akışı adımı yerine e-posta yanıt süresine bağımlıdır." },
        { title: "Tekrar eden veri girişi", desc: "Aynı talep bilgisi farklı roller tarafından farklı form, dosya veya sistemlerde tekrar girilir." },
        { title: "Sınırlı statü görünürlüğü", desc: "Ekipler 'bu talep şu an nerede?' sorusunu başka bir ekibe sormadan cevaplayamaz." },
        { title: "Ekipler arası bilgi kopukluğu", desc: "Procurement, finance ve requester ortak bir sistem durumu yerine gayri resmi güncellemelere güvenir." },
        { title: "Süreç performansını ölçmek zor", desc: "Tutarlı statü verisi olmadan çevrim süresi ve darboğaz analizi nitel kalır." },
      ],
      analysisApproach:
        "Mevcut süreci rol ve adım bazında haritalarım; karar noktalarını bekleme alanlarından ayrıştırırım; tekrar eden veri girişlerini ve manuel kontrolleri işaretlerim; gecikmeye neden olan ama görünmeyen noktaları açık hale getiririm. Ardından daha sade ve sistem destekli bir hedef süreç öneririm ve önerilen yapının UAT açısından test edilebilir davranışlar üretip üretmediğini kontrol ederim.",
      analysisApproachItems: [
        "Mevcut süreci rol ve adım bazında haritaladım",
        "Karar noktalarını bekleme alanlarından ayrıştırdım",
        "Tekrar eden veri girişlerini ve manuel kontrolleri işaretledim",
        "Görünmeyen ama gecikmeye neden olan noktaları açık hale getirdim",
        "Daha sade ve sistem destekli bir hedef süreç önerdim",
        "Hedef yapının UAT'ta doğrulanabilir kalmasına dikkat ettim",
      ],
      layers: [
        { title: "Rol Katmanı", desc: "Her adımın sahibinin kim olduğu ve sorumluluğun ekipler arasında nerede değiştiği" },
        { title: "Adım Katmanı", desc: "Süreci ilerleten gerçek aksiyonlar ile pasif bekleme dönemleri" },
        { title: "Karar Katmanı", desc: "Sürecin yolunu değiştiren onay, red ve yönlendirme kararları" },
        { title: "Handoff Katmanı", desc: "İşin ekipler arasında devredildiği ve bilgi kaybının en sık oluştuğu noktalar" },
        { title: "Statü Görünürlüğü Katmanı", desc: "Mevcut durumun nasıl iletildiği ve kimler tarafından gözlemlenebildiği" },
      ],
      businessRules: [
        { id: "PR-01", rule: "Her talep tek bir yapılandırılmış giriş noktasından sürece dahil olmalıdır", rationale: "Paralel kanalları ortadan kaldırmak ve baştan itibaren izlenebilirlik sağlamak" },
        { id: "PR-02", rule: "Zorunlu talep alanları, talep ilerlemeden önce doğrulanmalıdır", rationale: "Eksik verinin sürece geçip sonradan tekrar girilmesini önlemek" },
        { id: "PR-03", rule: "Onaylar e-posta yerine sistem üzerinden yönlendirilmelidir", rationale: "Onay süresini ölçülebilir kılmak ve gayri resmi iletişimi engellemek" },
        { id: "PR-04", rule: "Tedarikçi uygunluk kontrolleri talep kaydı üzerinde kayıt altına alınmalıdır", rationale: "Procurement işinin tekrarlanmasını veya kaybolmasını engellemek" },
        { id: "PR-05", rule: "Satın alma kaydı (PO) aynı talep kaydından üretilmelidir", rationale: "Mükerrer veri girişini ve sonradan yapılacak mutabakat yükünü ortadan kaldırmak" },
        { id: "PR-06", rule: "Süreç statüsü merkezi olarak güncellenmeli ve ilgili tüm rollere görünür olmalıdır", rationale: "Gayri resmi statü paylaşımını tek bir güvenilir kaynakla değiştirmek" },
      ],
      flowTitle: "Önerilen TO-BE Süreç Akışı",
      flowSteps: [
        "Requester talebi zorunlu alanları doğrulanmış şekilde sistemde oluşturur",
        "Manager onay görevini platform üzerinden alır",
        "Onayın ardından tedarikçi uygunluk kontrolü adımı tetiklenir",
        "Procurement tedarikçi onayını aynı talep kaydı üzerinde işler",
        "Satın alma kaydı (PO) aynı akıştan, yeniden veri girişi olmadan üretilir",
        "Statü her geçişte merkezi olarak güncellenir",
        "Finance, son sistem durumuna göre kaydı tamamlar",
      ],
      operationalQuestions: [
        "Bu talep şu anda süreçte nerede?",
        "Sonraki sorumlu rol kim?",
        "Onay tamamlandı mı?",
        "Tedarikçi uygunluğu doğrulandı mı?",
        "Çok uzun süredir bekleyen bir adım var mı?",
        "Bu talepte en çok hangi adım gecikmeye neden oldu?",
      ],
      screenSections: [
        "Talep Özeti",
      ...
      testApproach:
        "Test, yeniden tasarlanan akışın her handoff noktasında doğru davranıp davranmadığına odaklanır: talep oluşturmada zorunlu alan doğrulaması, onay yönlendirmesi, tedarikçi uygunluk adımı, PO'nun aynı kayıttan üretilmesi, her geçişte merkezi statü güncellemesi ve finans kaydının sistem durumuyla tutarlılığı.",
      uatApproach:
        "UAT'ta her rolün yeni süreci öncekinden daha net deneyimleyip deneyimlemediği doğrulanır: talebin nerede olduğunu görebiliyor mu, kendi adımını doğru zamanda alıyor mu, daha önce girilmiş bilgiyi tekrar girmek zorunda kalıyor mu ve uçtan uca statü ekipler arasında tutarlı mı.",
      uatChecklist: [
        "Requester talebin mevcut durumunu başka bir ekibe sormadan görebiliyor mu?",
        "Manager onay görevini zamanında platform üzerinden alıyor mu?",
        "Tedarikçi uygunluk adımı yalnızca onaydan sonra mı görünüyor?",
        "Satın alma kaydı (PO) talep verisi tekrar girilmeden üretiliyor mu?",
        "Statü her geçişten sonra merkezi olarak güncelleniyor mu?",
        "Finance kaydı yalnızca sistem durumuna bakarak tamamlayabiliyor mu?",
        "Ekipler arası handoff'lar aktivite geçmişinde görünüyor mu?",
      ...
      expectedOutput: [
        ...
        "Daha düşük hata ve unutma riski",
        "Roller arası daha net statü görünürlüğü",
        "Ekipler arası daha açık sorumluluk dağılımı",
        "Daha kolay test ve UAT doğrulaması",
        "Daha sürdürülebilir operasyonel yapı",
      ],
      demonstrates: [
        "Mevcut durum ile hedef durum arasında net bir karşılaştırma yapabilme",
        "Süreçleri yalnızca tarif etmek yerine iyileştirme mantığıyla ele alma",
        "Operasyonel görünürlük ve karar netliği üretebilme",
        "Analizi görsel ve yapısal deliverable'a dönüştürebilme",
        "İş ihtiyacı ile sistem davranışını aynı çerçevede okuyabilme",
      ],
      testScenarios: [
        { id: "PR-TS-01", scenario: "Talep zorunlu bir alan olmadan gönderilir", expected: "Sistem gönderimi engeller ve eksik alanı gösterir" },
        { id: "PR-TS-02", scenario: "Manager onayı platform üzerinden verilir", expected: "Süreç otomatik olarak tedarikçi uygunluk adımına ilerler" },
        { id: "PR-TS-03", scenario: "Tedarikçi uygunluğu sistemde doğrulanır", expected: "Doğrulama aynı talep üzerinde, yeniden girişe gerek kalmadan kaydedilir" },
        ...
        { id: "PR-TS-06", scenario: "Finance PO üretiminden sonra talebi açar", expected: "Son kayıt, merkezi sistem durumunu yansıtır" },
      ],
      processMap: {
        ...
        asIs: {
          ...
          notes: [
            "E-posta üzerinden çok sayıda handoff",
          ...
          lanes: [
            { role: "Requester", icon: "requester", steps: ["Talebi manuel oluştur", "E-posta ile gönder"] },
            { role: "Manager", icon: "manager", steps: ["Talebi incele", "E-posta ile onayla"] },
            { role: "Procurement", icon: "procurement", steps: ["Tedarikçi uygunluğunu manuel kontrol et", "PO'yu ayrı oluştur", "Tedarikçiyle iletişime geç", "Onayı al"] },
            { role: "Finance", icon: "finance", steps: ["PO'yu sonradan sisteme kaydet"] },
          ],
        },
        toBe: {
          ...
          notes: [
            ...
            "Daha güvenilir alt akışlar",
          ],
          lanes: [
            { role: "Requester", icon: "requester", steps: ["Talebi sistemde oluştur", "Zorunlu alanlar doğrulanır"] },
            { role: "Manager", icon: "manager", steps: ["Onay görevini al", "Platform üzerinden onayla"] },
            { role: "Procurement", icon: "procurement", steps: ["Tedarikçi uygunluğu kontrol edilir", "PO aynı akıştan üretilir", "Onay kayıt altına alınır"] },
            { role: "Finance", icon: "finance", steps: ["Sonuç merkezi durumdan kaydedilir"] },
          ],
        },
      },
    },
